...
import Utils (outOfBounds, neighbors, runParser)
...
bfs grid end goUp start = bfs' grid end (Set.singleton start) [(0, start)] goUp

bfs' :: Grid -> Int -> Set.Set Point -> [(Int, Point)] -> Bool -> Int
bfs' _ _ _ [] _ = maxBound -- We ran out of places to visit
bfs' grid end visited ((distance, pos@(x, y)) : toVisit) goUp
  ...
  | otherwise =
      let newPoints = filter traversable $ neighbors pos
       ...
       in bfs' grid end newVisited newToVisit goUp
  where
