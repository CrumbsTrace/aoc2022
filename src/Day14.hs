...
run input = (p1, p2)
  where
    ...
    lowestRock = maximum $ map snd $ Set.elems cave
...
dropSand occupiedSet p@(x, y) lowestRock noFloor path
  ...
  | Set.notMember (x, y + 1) occupiedSet = dropSand occupiedSet (x, y + 1) lowestRock noFloor (p : path)
  | Set.notMember (x - 1, y + 1) occupiedSet = dropSand occupiedSet (x - 1, y + 1) lowestRock noFloor (p : path)
  | Set.notMember (x + 1, y + 1) occupiedSet = dropSand occupiedSet (x + 1, y + 1) lowestRock noFloor (p : path)
  | (x, y) == (500, 0) && Set.member (500, 0) occupiedSet = []
  | otherwise = p : path

parser :: Parser (Set.Set Point)
